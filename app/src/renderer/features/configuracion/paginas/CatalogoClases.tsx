...
import { Aviso, Boton, Campo, Cargando, Casilla, Encabezado, EstadoVacio, Insignia, TablaSimple, AreaTexto } from '../../../componentes/ui';
...
const INVALIDA = ['clase:listar', ...EFECTOS_CONFIGURACION] as const;
...
export function CatalogoClases(): JSX.Element {
  ...
  const clases = useCanal('clase:listar', { procesoId, incluirInactivas: true });
  ...
  return (
    <>
      <Encabezado
        ...
        acciones={
          <>
            <ImportarPlantilla procesoId={procesoId} plantilla="PL-02" invalida={['clase:listar']} />
            <Boton icono={<Sparkles className="h-4 w-4" aria-hidden />} cargando={precargar.isPending} onClick={() => precargar.mutate({ procesoId })}>
              Precargar sugeridas
            </Boton>
      ...
      <TablaSimple
        columnas={[
          ...
          { clave: 'resp', titulo: 'Responsable técnico', celda: (c) => c.responsableTecnico },
        ...
        filas={lista}
        claveFila={(c) => c.id}
        vacio={<EstadoVacio titulo="Sin clases de activo" descripcion="Precargue el catálogo sugerido, importe PL-02 o cree las clases una a una." accion={<Boton variante="primario" onClick={() => precargar.mutate({ procesoId })}>Precargar sugeridas</Boton>} />}
      />
      <DialogoClase procesoId={procesoId} clase={editando} onCerrar={() => setEditando(null)} />
    </>
  );
}

function DialogoClase({ procesoId, clase, onCerrar }: { procesoId: string; clase: ClaseActivoDto | 'nueva' | null; onCerrar: () => void }): JSX.Element {
  ...
  const valores: Partial<Valores> = esNueva || clase === null
    ? { esDepreciable: true, requiereHojaVida: false, requiereInvima: false, activo: true, responsableTecnico: '' }
    : { ...clase };
  ...
  const guardar = (v: Validados): void => {
    ...
    if (esNueva) crear.mutate({ procesoId, ...campos }, { onSuccess: onCerrar });
    ...
    <Dialogo abierto={clase !== null} onCambioAbierto={(a) => { if (!a) onCerrar(); }} titulo={esNueva ? 'Nueva clase de activo' : `Editar clase ${clase === null ? '' : clase.codigo}`} ancho="lg">
      <form id="form-clase" noValidate onSubmit={handleSubmit(guardar)} className="flex flex-col gap-4">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <Campo etiqueta="Responsable técnico" obligatorio ayuda="Área que responde por esta clase, p. ej. Ingeniería biomédica" error={errors.responsableTecnico?.message} {...register('responsableTecnico')} />
        </div>
        ...
        {!esNueva && <AreaTexto etiqueta="Justificación del cambio" ayuda="Vale para los cálculos que vengan; los cortes ya hechos conservan la suya (ANEXO_C §10). Queda en bitácora." error={errors.justificacion?.message} {...register('justificacion')} />}
