...
import { entregarPlantilla, type CatalogosProceso, type DatosMembrete } from '../../../infraestructura/documental/excel/entregarPlantilla';
...
function catalogosDe(ctx: ContextoIpc, procesoId: string | null): CatalogosProceso {
  if (procesoId === null) return { clases: [], sedes: [], servicios: [] };
  const columna = (sql: string): string[] => (ctx.sqlite.prepare(sql).all(procesoId) as { v: string }[]).map((f) => f.v);
  return {
    clases: columna(`SELECT nombre AS v FROM clase_activo WHERE proceso_id = ? AND activo = 1 ORDER BY codigo`),
    sedes: columna(`SELECT codigo AS v FROM sede WHERE proceso_id = ? AND activa = 1 ORDER BY codigo`),
    servicios: columna(`SELECT DISTINCT v.nombre AS v FROM servicio v JOIN sede s ON s.id = v.sede_id WHERE s.proceso_id = ? AND v.activo = 1 AND s.activa = 1 ORDER BY v.nombre`),
  };
}

function membreteDe(ctx: ContextoIpc, procesoId: string | null): DatosMembrete {
  const entidad = procesoId === null ? undefined : (ctx.sqlite.prepare('SELECT razon_social FROM proceso WHERE id = ?').get(procesoId) as { razon_social: string } | undefined);
  return { razonSocial: entidad?.razon_social ?? '' };
}
...
export async function descargarPlantilla(e: EntradaValidadaDe<'plantilla:descargar'>, ctx: ContextoIpc): Promise<ResultadoDescargaPlantilla | null> {
  ...
  const r = await entregarPlantilla(plantilla, rutaOrigen(ctx, plantilla), destino, catalogosDe(ctx, e.procesoId ?? null), membreteDe(ctx, e.procesoId ?? null));
...
export async function descargarPaquetePlantillas(
  ...
  const catalogos = catalogosDe(ctx, e.procesoId ?? null);
  const membrete = membreteDe(ctx, e.procesoId ?? null);
