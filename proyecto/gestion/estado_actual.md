...
| Qué | Dónde |
|---|---|
| **De 7 carpetas en la raíz a 4** | `especificacion/` (teoria, normatividad, plantillas) · `proyecto/` (plan, gestion) · `app/` · `datos_de_prueba/`. Con `git mv`, así que el historial se conserva |
| **`/Teoria` también se movió** | Lo decidió el propietario. Su contenido no cambió: solo las rutas de `CORRECCIONES.md`, con una nota fechada |
| **Código** | `electron-builder.yml`, `rutaPlantillas`, scripts (`sembrar`, `datos:prueba`, `instructivo`, `trazabilidad`) y los tests. Empaquetadas, las plantillas siguen en `resources/plantillas` |
| **Verificado** | `verificar:todo` en verde. 0 enlaces rotos antes y después. **No se corrieron los E2E ni `pack`** |

**Antes**, el 2026-09-04: **documentación técnica de `/app`**.
...
| **Enlazado** | Fila nueva en el mapa de `CLAUDE.md`, junto a `especificacion/normatividad/` |

**Antes**, el mismo día: **verificación normativa y ADR-027**.

| Qué | Dónde |
|---|---|
| **`/especificacion/normatividad`** | Carpeta nueva: 5 fichas + índice contrastando el proceso con la norma colombiana. Cada afirmación marcada ✅ verificada / ⚠️ indirecta / ❓ por confirmar, y una lista explícita de lo que **no** se verificó |
...
| **CT-20** | `umbral_semaforo_naranja` existía en `ANEXO_B` pero `ANEXO_C` §2.6 usaba un 1,0 literal. Manda el parámetro. `/especificacion/teoria` **no se tocó** |
...
| Qué | Dónde |
|---|---|
| **ADR-026 · reenfoque** | `proyecto/plan/DECISIONES/adr_producto.md`. `/especificacion/teoria` intacta; cambia el orden. Núcleo + extensiones. El backlog maestro lo advierte en cabecera |
...
1. **`/especificacion/teoria` manda**, versión **2.1**; `ANEXO_C` prevalece en cálculo.
